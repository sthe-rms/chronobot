...
var save_files_name = "settings_data.tres"

func load_settings():
	if !DirAccess.dir_exists_absolute(save_settings_path):
		DirAccess.make_dir_absolute(save_settings_path)
		
	if ResourceLoader.exists(save_settings_path + save_files_name):
		settings_data = ResourceLoader.load(save_settings_path + save_files_name)
...
func save_settings():
	ResourceSaver.save(settings_data, save_settings_path + save_files_name)
